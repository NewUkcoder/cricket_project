...
<head>
    ...
    <title>Cricket Scorecard Admin - Second Innings</title>
...
<body>
    ...
    try {
        ...
        $bowling_second = $toss_id['bat_first'] ?? 0;
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php 
                    if (empty($bat_second)) {
                        echo '<tr><td colspan="7" class="text-center">Add batting record of second innings</td></tr>';
                    } else {
                        try {
                            foreach($bat_second as $bat) {
                                $strike_rate = ($bat->balls > 0) ? round(($bat->runs / $bat->balls) * 100, 2) : 0;
                                // Debug: Log if player_id is missing
                                if (!isset($bat->player_id)) {
                                    error_log("Missing player_id for player: " . ($bat->playerName ?? 'Unknown'));
                                }
                                ?>
                                <tr>
                                    <td class="text-start">
            ...
            <div class="action-btn-container">
                ...
                <a href="<?php echo base_url();?>Welcome/scorecard_links/<?php echo htmlspecialchars($bowling_second ?? ''); ?>/<?php echo htmlspecialchars($batting_second ?? ''); ?>/<?php echo htmlspecialchars($match_id ?? ''); ?>">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?php echo base_url();?>/ScorecardController/insert_batting" method="POST" id="scorecardForm" onsubmit="return validateScorecard()">
                        ...
                        <input type="hidden" value="<?php echo htmlspecialchars($bowling_second ?? ''); ?>" name="bowling_team">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?php echo base_url();?>/ScorecardController/insert_extras" method="POST" id="extrasForm">
                        <input type="hidden" value="<?php echo htmlspecialchars($match_id ?? ''); ?>" name="match_id">
                        <input type="hidden" value="<?php echo htmlspecialchars($batting_second ?? ''); ?>" name="batting_team_id">
                        <input type="hidden" value="2" name="batting_order">
                        <div class="mb-3">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?php echo base_url();?>/ScorecardController/edit_score" method="POST" id="editScorecardForm" onsubmit="return validateEditScorecard()">
                        ...
                        <input type="hidden" id="edit-player-id" name="player_id" value="">
                        <input type="hidden" value="2" name="batting_order">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                        ...
                        <div class="mb-3" id="edit-bowler-section">
                            ...
                            <select class="form-select" id="edit-bowler-name" name="bowler_id">
                                ...
                                <?php foreach($bowler_info as $bowler) { ?>
                                    <option value="<?php echo htmlspecialchars($bat->player_id ?? ''); ?>">
                                        <?php echo htmlspecialchars($bowler->playerName ?? ''); ?>
                                    </option>
                                <?php } ?>
                            </select>
    ...
    <script>
        ...
        function validateScorecard() {
            ...
            if (!playerId) {
                alert('Player ID is missing. Please select a player.');
                return false;
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('editScorecardModal').addEventListener('show.bs.modal', function(event) {
                ...
                // Explicitly set match_id and player_id
                ...
                Object.entries(fields).forEach(([id, attr]) => {
                    if (id !== 'match-id' && id !== 'player-id') { // Skip match-id and player-id as they're handled separately
